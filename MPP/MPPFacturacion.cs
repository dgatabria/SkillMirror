...
namespace MPP
{
    ...
    public class MPPFacturacion
    {
        ...
        {
            // 1. Instanciamos la capa de acceso a datos
            Acceso oDatos = new Acceso();

            // 2. Ejecutamos el SP. No tiene parámetros, así que pasamos null.
            // Usamos el método LeerSP de tu clase Acceso.cs
            DataTable dt = oDatos.LeerSP("sp_ObtenerEstadisticasGanancias", null);

            BEEstadisticasGanancias estadisticas = new BEEstadisticasGanancias();

            // 3. Verificamos que la consulta haya devuelto al menos una fila
            // (El SP siempre devuelve una fila, pero es buena práctica chequear)
            if (dt.Rows.Count > 0)
            {
                // 4. Tomamos la primera (y única) fila de resultados
                DataRow row = dt.Rows[0];

                // 5. Mapeamos las columnas del DataRow a las propiedades del objeto BE.
                // Es crucial hacer el cast a 'decimal', ya que MontoTotal es decimal.
                estadisticas.GananciasHoy = (decimal)row["GananciasHoy"];
                estadisticas.GananciasSemana = (decimal)row["GananciasSemana"];
                estadisticas.GananciasMes = (decimal)row["GananciasMes"];
                estadisticas.GananciasAnio = (decimal)row["GananciasAnio"];
            }

            // 6. Devolvemos el objeto (lleno o vacío/en ceros)
            return estadisticas;
        }
        /// <summary>
